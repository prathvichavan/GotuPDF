...
export default function RemovePagesTool() {
    ...
    const handleFileSelect = useCallback(async (selectedFiles: File[]) => {
        ...
        setDownloadBaseName(parts.base ? parts.base + "-pages-removed" : "pages-removed");
    ...
    const handleDownload = () => {
        ...
        a.href = downloadUrl;
        a.download = buildDownloadName(downloadBaseName, downloadExtension, "output.pdf");
        a.click();
    };
        ...
        <div className="min-h-screen p-4 md:p-8">
            <div className="max-w-4xl mx-auto">
                <div className="text-center mb-12">
                    <h1 className="text-5xl font-bold text-gray-900 dark:text-white mb-4">Remove Pages</h1>
                    ...
                    <>
                        <div className="bg-gray-100 dark:bg-white/5 rounded-2xl shadow-xl shadow-black/10 dark:shadow-black/30 p-6 mb-8">
                            ...
                            <div className="flex gap-2 mb-4">
                                <button onClick={selectAll} className="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors text-sm font-medium">
                                    Select All
                                </button>
                                <button onClick={deselectAll} className="px-4 py-2 bg-gray-300 dark:bg-white/50 text-gray-700 dark:text-white rounded-lg hover:bg-gray-400 dark:hover:bg-white/15 transition-colors text-sm font-medium">
                                    Deselect All
                                </button>
                            </div>
                                    ...
                                    <button
                                        ...
                                        className={`aspect-square rounded-lg border-2 font-semibold transition-all focus:outline-none text-sm ${
                                            selectedPages.includes(pageNum)
                                                ...
                                                : "border-gray-300 dark:border-white/15 bg-gray-100 dark:bg-white/5 text-gray-700 dark:text-white hover:border-red-400 hover:bg-red-500/10"
                                        }`}
                                    >
                                        {pageNum}
                                    </button>
                            ...
                            {selectedPages.length > 0 && selectedPages.length < totalPages && (
                                <div className="text-center text-gray-500 dark:text-slate-400 mb-4">
                                    {totalPages - selectedPages.length} pages will remain after removal
                                </div>
                            )}
                        </div>
                        ...
                        {isProcessing && (
                            <div className="bg-gray-100 dark:bg-white/5 rounded-2xl shadow-xl shadow-black/10 dark:shadow-black/30 p-8 mb-8">
                                <div className="text-center mb-4">
                                    <div className="text-xl font-semibold text-gray-600 dark:text-slate-300">Removing pages...</div>
                                </div>
                        ...
                        {!isProcessing && selectedPages.length > 0 && selectedPages.length < totalPages && (
                            <button
                                onClick={handleProcess}
                                className="w-full bg-gradient-to-r from-red-500 to-orange-500 text-white text-xl font-bold py-6 rounded-2xl hover:from-red-600 hover:to-orange-600 transition-all shadow-xl shadow-black/10 dark:shadow-black/30 hover:shadow-2xl transform hover:scale-105"
                            >
                ...
                {downloadUrl && (
                    <div className="bg-gray-100 dark:bg-white/5 rounded-2xl shadow-xl shadow-black/10 dark:shadow-black/30 p-8 text-center">
                        ...
                        <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Pages Removed!</h3>
                        <p className="text-gray-500 dark:text-slate-400 mb-6">
                            Removed {selectedPages.length} page{selectedPages.length > 1 ? "s" : ""}. Your PDF is ready to download.
                        </p>
                        <div className="max-w-md mx-auto mb-6 text-left">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                />
                                <span className="px-3 py-2 bg-gray-100 dark:bg-white/5/10 rounded-lg text-sm text-gray-500 dark:text-slate-400">.pdf</span>
                            </div>
                        </div>
                        <div className="flex flex-col sm:flex-row gap-4 justify-center">
                            ...
                            <button onClick={handleReset} className="px-8 py-4 bg-gray-100 dark:bg-white/5/10 text-gray-600 dark:text-slate-300 rounded-xl font-semibold hover:bg-gray-200 dark:hover:bg-white/5/15 transition-all">
                                Remove More Pages
                            </button>
                        </div>
                    </div>
